...
def check_login():
    ...
    if "users" not in st.secrets:
        st.warning("⚠️ No users configured in .streamlit/secrets.toml. Please add a [users] section.")
        st.stop()
        return False
    ...
    def login_entered():
        ...
        if user in st.secrets["users"] and st.secrets["users"][user] == passwd:
    ...
    if not st.session_state["logged_in"]:
        ...
        st.text_input("Password", type="password", key="password_input", on_change=login_entered)
        if st.button("Login"):
            login_entered()
        return False
    else:
